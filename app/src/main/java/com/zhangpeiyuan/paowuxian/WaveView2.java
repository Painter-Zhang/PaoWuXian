...
public class WaveView2 extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (!isMeasured) {
            ...
            // 水位线从最底下开始上升
            mLevelLine = getMeasuredHeight() / 2;
            // 根据View宽度计算波形峰值
            mWaveHeight = mViewWidth / 6f;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        path.addCircle(getHeight() / 2, getHeight() / 2, getWidth() / 2, Direction.CCW);
        ...
        // 绘制百分比
        canvas.drawText("" + ((int) ((1 - mLevelLine / getHeight()) * 100))
                + "%", getWidth() / 2, mLevelLine + mWaveHeight
                + (getHeight() - mLevelLine - mWaveHeight) / 2, mTextPaint);
